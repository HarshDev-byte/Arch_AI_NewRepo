...
export type RoomType =
  | "living" | "dining" | "kitchen" | "master_bedroom"
  | "bedroom" | "bathroom" | "balcony" | "staircase"
  | "study" | "utility" | "corridor" | "pooja";
...
const ROOM_TYPE_OPTIONS: RoomType[] = [
  ...
  "bathroom", "balcony", "staircase", "study", "utility", "corridor", "pooja"
];
...
export default function FloorPlanEditor({
  initialRooms, plotWidthM, plotDepthM, plotAreaSqm,
  fsiAllowed, floors, onSave, onRegenerate3D
  ...
  // Live stats
  const totalBuiltUp = allRooms.reduce((s, r) => s + r.w * r.h, 0) * floors;
  ...
  const checkCompliance = useCallback((): ComplianceResult => {
    ...
    if (!visibleRooms.some(r => r.type === "staircase") && floors > 1) {
      warnings.push("No staircase defined — required for multi-floor building");
    }
    if (!visibleRooms.some(r => ["bathroom", "master_bedroom"].includes(r.type))) {
      warnings.push("No bathroom found on this floor");
    }
    return { passed: issues.length === 0, fsiOk, fsiUsed, fsiAllowed, issues, warnings };
  }, [rooms, fsiUsed, fsiAllowed, plotAreaSqm, plotWidthM, plotDepthM, floors, visibleRooms]);
  ...
  const handleStageMouseMove = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (w > GRID && h > GRID) {
      setDrawPreview({ id: "preview", name: "New room", type: "bedroom", x, y, w, h, floor: activeFloor });
    }
  };

  const handleStageMouseUp = () => {
    if (tool !== "draw" || !drawPreview) { setDrawStart(null); setDrawPreview(null); return; }
    const newRoom: Room = { ...drawPreview, id: uuid(), name: "New room" };
  ...
  const handleDragEnd = (id: string, e: Konva.KonvaEventObject<DragEvent>) => {
    ...
    setRooms(rooms.map(r => r.id === id ? { ...r, x: newX, y: newY } : r));
  };
  ...
  const DimLabel = ({ room }: { room: Room }) => {
    const cx = mToPx(room.x + room.w / 2);
    const cy = mToPx(room.y + room.h / 2);
    return (
      <>
        <Text
          x={mToPx(room.x)} y={mToPx(room.y) - 14}
          width={mToPx(room.w)} text={`${room.w.toFixed(1)}m`}
          fontSize={9} fill="#185FA5" align="center"
        />
        <Text
          x={mToPx(room.x) - 24} y={mToPx(room.y)}
          width={22} height={mToPx(room.h)}
          text={`${room.h.toFixed(1)}m`}
          fontSize={9} fill="#185FA5"
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center", flexWrap: "wrap" }}>
          {(["select", "draw", "delete"] as const).map(t => (
            ...
            }}>{t === "select" ? "Select" : t === "draw" ? "+ Draw room" : "Delete"}</button>
          ))}
          <div style={{ width: 1, height: 20, background: "var(--color-border-tertiary)" }} />
          <button onClick={undo} disabled={!canUndo} style={{ padding: "4px 10px", fontSize: 12, borderRadius: 8, cursor: canUndo ? "pointer" : "default", border: "0.5px solid var(--color-border-secondary)", background: "var(--color-background-secondary)", opacity: canUndo ? 1 : 0.4, color: "var(--color-text-secondary)" }}>Undo</button>
          <button onClick={redo} disabled={!canRedo} style={{ padding: "4px 10px", fontSize: 12, borderRadius: 8, cursor: canRedo ? "pointer" : "default", border: "0.5px solid var(--color-border-secondary)", background: "var(--color-background-secondary)", opacity: canRedo ? 1 : 0.4, color: "var(--color-text-secondary)" }}>Redo</button>
          <div style={{ width: 1, height: 20, background: "var(--color-border-tertiary)" }} />
        ...
        <div style={{ border: "0.5px solid var(--color-border-secondary)", borderRadius: 12, overflow: "hidden" }}>
          <Stage
            ...
            style={{ background: "#faf9f6", cursor: tool === "draw" ? "crosshair" : tool === "delete" ? "not-allowed" : "default" }}
            onMouseDown={handleStageMouseDown}
            onMouseMove={handleStageMouseMove}
            onMouseUp={handleStageMouseUp}
            onClick={() => { if (tool === "select") { setSelectedId(null); setPropPanel(null); } }}
          >
            <Layer>
              {/* Grid */}
              ...
              <Rect
                x={PAD} y={PAD}
                width={mToPx(plotWidthM)} height={mToPx(plotDepthM)}
                fill="transparent" stroke="rgba(0,0,0,0.2)" strokeWidth={2}
              />
              ...
              {visibleRooms.map(room => {
                ...
                return (
                  <Group
                    key={room.id}
                    x={PAD + mToPx(room.x)} y={PAD + mToPx(room.y)}
                  ...
                  >
                    <Rect
                      width={mToPx(room.w)} height={mToPx(room.h)}
                      fill={ROOM_COLORS[room.type]}
                      stroke={isSel ? "#185FA5" : ROOM_STROKE[room.type]}
                    ...
                    <Text
                      ...
                      align="center" fontStyle={isSel ? "500" : "normal"}
                    />
                    <Text
                      width={mToPx(room.w)}
                      y={mToPx(room.h) / 2 + 3}
                      text={`${(room.w * room.h).toFixed(1)} m²`}
                      fontSize={9} fill="rgba(0,0,0,0.35)" align="center"
                    />
              ...
              {drawPreview && (
                <Rect
                  x={PAD + mToPx(drawPreview.x)} y={PAD + mToPx(drawPreview.y)}
                  width={mToPx(drawPreview.w)} height={mToPx(drawPreview.h)}
              ...
              />
              <Text x={PAD} y={PAD + mToPx(plotDepthM) + 22} width={mToPx(5)}
                text="5 m" fontSize={9} fill="rgba(0,0,0,0.35)" align="center" />

              {/* Compass */}
              <Text x={PAD + 4} y={PAD + 4} text="N ↑" fontSize={9} fill="rgba(0,0,0,0.3)" />
            </Layer>
          </Stage>
        </div>
      </div>
      ...
      <div style={{ minWidth: 200, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {compliance && (
          <div style={{
            ...
            <div style={{ fontSize: 13, fontWeight: 500, color: compliance.passed ? "#27500a" : "#791f1f", marginBottom: 5 }}>
        ...
        {propPanel && (
          <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, padding: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
              {(["w", "h"] as const).map(dim => (
                <div key={dim}>
                  ...
                  <input
                    type="number" step={GRID} min={GRID}
            ...
            <button
              onClick={() => { setRooms(rooms.filter(r => r.id !== propPanel.id)); setSelectedId(null); setPropPanel(null); }}
              style={{ width: "100%", padding: "6px", fontSize: 12, borderRadius: 8, border: "0.5px solid var(--color-border-danger)", background: "transparent", color: "var(--color-text-danger)", cursor: "pointer" }}
            >
              Delete room
            </button>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <button
            onClick={handleSave}
            disabled={saving}
            style={{ padding: "8px", fontSize: 13, borderRadius: 8, border: "0.5px solid var(--color-border-secondary)", background: "var(--color-background-secondary)", cursor: "pointer", color: "var(--color-text-primary)" }}
          >
          ...
          <button
            ...
            style={{ padding: "8px", fontSize: 13, borderRadius: 8, border: compliance?.passed ? "0.5px solid #185FA5" : "0.5px solid var(--color-border-tertiary)", background: compliance?.passed ? "#E6F1FB" : "var(--color-background-secondary)", cursor: compliance?.passed ? "pointer" : "default", color: compliance?.passed ? "#0C447C" : "var(--color-text-secondary)" }}
          >
            Regenerate 3D model
          </button>
          <button
            onClick={() => exportDXF(rooms, plotWidthM, plotDepthM)}
            style={{ padding: "8px", fontSize: 13, borderRadius: 8, border: "0.5px solid var(--color-border-secondary)", background: "transparent", cursor: "pointer", color: "var(--color-text-secondary)" }}
          >
            Export DXF
          </button>
        </div>
...
function exportDXF(rooms: Room[], plotW: number, plotH: number) {
  const scale = 1000; // mm
  let dxf = `0\nSECTION\n2\nENTITIES\n`;
  rooms.forEach(r => {
