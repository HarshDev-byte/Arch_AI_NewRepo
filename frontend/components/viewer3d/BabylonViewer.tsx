...
export default function BabylonViewer({ modelUrl, sceneGraph, height = "500px" }: BabylonViewerProps) {
  ...
  useEffect(() => {
    ...
    // Wait for BABYLON to be loaded
    const checkBabylon = setInterval(() => {
      if (window.BABYLON) { clearInterval(checkBabylon); init(); }
    }, 100);
  ...
  return (
    <div className="relative rounded-xl overflow-hidden bg-[#0a0e1a]" style={{ height }}>
      <Script src="https://cdn.babylonjs.com/babylon.js"                        strategy="afterInteractive" />
      <Script src="https://cdn.babylonjs.com/loaders/babylonjs.loaders.min.js"  strategy="afterInteractive" />
      ...
      {/* Loading overlay */}
      <div id="babylon-loader" className="absolute inset-0 flex items-center justify-center bg-[#0a0e1a] pointer-events-none" style={{ display: "none" }}>
